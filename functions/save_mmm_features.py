...
def get_metadata():
    with open('data/mmm_metadata.json','r') as data:
...
def save_important_metadata(meta):    
    imp = [{
            ...
            "max_minus_hp":max_points.get(value['onchain_metadata']['Class'], None)-int(value['onchain_metadata']['Hit Points']),
           
            } for k, value in meta.items()]
    
    with open('data/mmm_features.json', 'w') as features:
